...
export default function NextEventSummary({ groups, allowMultipleOpen }: NextEventSummaryProps) {
  ...
  useEffect(() => {
    ...
    setExpandedKeys((currentKeys) => {
      if (allowMultipleOpen) {
        return currentKeys.length > 0 ? currentKeys : [firstGroupKey];
      }

      return currentKeys.includes(firstGroupKey) ? [firstGroupKey] : [firstGroupKey];
    });
  }, [allowMultipleOpen, firstGroupKey]);

  function toggleGroup(key: string) {
    setExpandedKeys((currentKeys) => {
      const isAlreadyExpanded = currentKeys.includes(key);

      if (allowMultipleOpen) {
        return isAlreadyExpanded ? currentKeys.filter((currentKey) => currentKey !== key) : [...currentKeys, key];
      }

      return isAlreadyExpanded ? [] : [key];
    });
  }
  ...
  return (
    <View style={styles.section}>
      <View style={styles.headingBlock}>
        ...
        <Text style={styles.viewModeText}>{allowMultipleOpen ? 'Multiple month view is on.' : 'Single month view is on.'}</Text>
      </View>
                  ...
                  {group.products.map(({ product, analysis }) => (
                    <View key={product.id} style={styles.itemRow}>
                      <View style={styles.itemTextBlock}>
                        ...
                        <Text style={styles.itemMeta}>Stock: {analysis.totalStockUnits.toFixed(2)} units</Text>
